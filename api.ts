...
export abstract class PlatformDirsABC {
    ...
    constructor(appname?: string, appauthor?: string | false, version?: string, roaming: boolean = false, multipath: boolean = false, opinion: boolean = true, ensureExists: boolean = false) {
    ...
    protected _firstItemAsPathIfMultipath(directory: string): string {
        if (this.multipath) {
            directory = directory.split(path.delimiter)[0];
        }
        return directory;
    }
